...
class userLogin
{
	...
	public function check_userlogin () {
		...
		//Verifica se existe um cookie com a chave userdate
		//não pode ser HTTP POST
		...
		if ( $this->phpass->CheckPassword( $password, $fetch['password'] ) ) {
			...
			if ( $post ) {
				...
				$query = $this->db->query(
					'UPDATE users SET user_session_id = ? WHERE id_users = ?',
					array( $session_id, $id_users )
				);

				//Se setado "mantenha-me conectado"
				if( isset($remember_me)){
					//cria cookie
					setcookie('userdata',base64_encode($id_users).'&'.base64_encode($session_id),time()+60*60*24*265*10,'/');
				}
			}
	...
	protected function goto_login() {
		...
		if ( defined( 'HOME_URI' ) ) {
			...
			$login_uri  = HOME_URI . 'admin/login';
