...
var setUploader = function(path){
	...
	$('#newfolder').unbind().click(function(){
		...
		var foldername = 'Nova pasta';
		var msg = 'Insira o nome da nova pasta: <input id="fname" name="fname" type="text" value="' + foldername + '" />';
		...
		var getFolderName = function(v, m){
			...
			if(fname != ''){
				...
				$.prompt('Nome da pasta em branco.');
			}
		}
		
		$.prompt(msg, {
			callback: getFolderName,
			buttons: { 'Criar Pasta': 1, 'Cancelar': 0 }
		});		
	});	
}
...
var bindToolbar = function(data){
	...
	$('#fileinfo').find('button#delete').click(function(){
		if(deleteItem(data)) $('#fileinfo').html('<h1>Selecione um item no menu.</h1>');
	});
...
var renameItem = function(data){
	var finalName = '';
	var msg = 'Insira um novo nome para o arquivo: <input id="rname" name="rname" type="text" value="' + data['Filename'] + '" />';
...
var deleteItem = function(data){
	var isDeleted = false;
	var msg = 'Tem certeza que deseja excluir este arquivo?';
...
var addNode = function(path, name){
	...
	$.prompt('Arquivo inserido com sucesso.');
}
...
var addFolder = function(parent, name){
	...
	$.prompt('Nova pasta criada com sucesso.');
}
...
var getFileInfo = function(file){
	...
	template += '<button id="select" name="select" type="button" value="Select">Selecionar</button>';
	template += '<button id="download" name="download" type="button" value="Download">Download</button>';
	template += '<button id="rename" name="rename" type="button" value="Rename">Renomear</button>';
	template += '<button id="delete" name="delete" type="button" value="Delete">Excluir</button>';
	template += '</form>';
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + file, function(data){
		if(data['Code'] == 0){
			...
			if(data['Properties']['Width'] && data['Properties']['Width'] != '') properties += '<dt>Dimensões</dt><dd>' + data['Properties']['Width'] + 'x' + data['Properties']['Height'] + '</dd>';
			if(data['Properties']['Date Created'] && data['Properties']['Date Created'] != '') properties += '<dt>Criado</dt><dd>' + data['Properties']['Date Created'] + '</dd>';
			if(data['Properties']['Date Modified'] && data['Properties']['Date Modified'] != '') properties += '<dt>Modificado</dt><dd>' + data['Properties']['Date Modified'] + '</dd>';
			if(data['Properties']['Size'] && data['Properties']['Size'] != '') properties += '<dt>Tamanho</dt><dd>' + formatBytes(data['Properties']['Size']) + '</dd>';
	...
	$.getJSON(fileConnector + '?path=' + path + '&mode=getfolder&showThumbs=' + showThumbs, function(data){		
		...
		if(data){		
			if($('#fileinfo').data('view') == 'grid'){
				...
				result += '<thead><tr><th class="headerSortDown"><span>Nome</span></th><th><span>Dimensões</span></th><th><span>Tamanho</span></th><th><span>Modificado</span></th></tr></thead>';
				result += '<tbody>';
